...
class Data:
    def Database():
        ...
        if cursor.fetchone() is None:
            ...
            cursor.execute("INSERT INTO `Inventory` (ACC_ID, ITEM_NAME, ITEM_QTY, ITEMS_IN_USE, ITEMS_NOT_IN_USE) VALUES (?, ?, ?, ?, ?)",
...
class Exit:
    def Root():
        ...
        if result == 'yes':
            ...
            exit()
    def Home():
        result = tkMessageBox.askquestion('BRG-IMG','Are you sure you want to exit? Completing this action will close the application', icon="warning")
        if result == 'yes':
            Home.destroy()
            exit()
    def Form():
...
class Functions:
    ...
    def AddNew():
        Data.Database()      
        cursor.execute("INSERT INTO `Inventory` (ACC_ID, ITEM_NAME, ITEM_QTY, ITEMS_IN_USE, ITEMS_NOT_IN_USE) VALUES (?, ?, ?, ?, ?)",
                       (int(ACCOUNT_ID), str(ITEM_NAME.get()), int(ITEM_QTY.get()), int(ITEMS_IN_USE.get()),  (int(ITEM_QTY.get())-int(ITEMS_IN_USE.get()))))
        conn.commit()
        ITEM_NAME.set("")
        ITEM_QTY.set("")
        ITEMS_IN_USE.set("")
...
b2 = Button(root, font=("Bahnschrift Semibold", 10), text = 'Create Account', height = 2, width = 30,command = lambda : (Data.Database(), Forms.ShowCreateForm()))
