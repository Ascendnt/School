...
class Forms:
    ...
    #Labels
        ...
        Text.pack(fill=X)
        Username = Label(MidCreateForm, text="Username:", font=("Bahnschrift Semibold", 20), bd=18, fg = 'white', bg = '#006400')
        Username.grid(row=0)
    ...
    #Buttons
        Create = Button(MidCreateForm, text="Create Account", font=("Bahnschrift Semibold", 15), width=20, command=Functions.CreateAcc)
    ...
    #Labels
        ...
        itemnamelable = Label(MidAddNew, bg = '#006400', fg = 'white' , text='Item Name:', font=("Bahnschrift Semibold", 20), bd = 5)
        itemnamelable.grid(row=0, sticky=W)
        itemqtylable = Label(MidAddNew, bg = '#006400', fg = 'white' , text='Item Qty:', font=("Bahnschrift Semibold", 20), bd = 5)
        itemqtylable.grid(row=1, sticky=W)
        itemsinuselable = Label(MidAddNew, bg = '#006400', fg = 'white' , text='Item In Use:', font=("Bahnschrift Semibold", 20), bd = 5)
    ...
    #Labels
        namelabel = Label(TopEditNew,  bg = '#006400', fg = 'white' ,text="Edit Product", font=("Arial Black", 20), width=600)
        namelabel.pack(fill=X)
        itemnamelable = Label(MidEditNew, bg = '#006400', fg = 'white' , text='Item Name:', font=("Bahnschrift Semibold", 20), bd = 5)
        itemnamelable.grid(row=0, sticky=W)
        itemqtylable = Label(MidEditNew, bg = '#006400', fg = 'white' , text='Item Qty:', font=("Bahnschrift Semibold", 20), bd = 5)
        itemqtylable.grid(row=1, sticky=W)
        itemsinuselable = Label(MidEditNew, bg = '#006400', fg = 'white' , text='Item In Use:', font=("Bahnschrift Semibold", 20), bd = 5)
        itemsinuselable.grid(row=2, sticky=W)
    ...
    #Button
        searchbutton = Button(LeftViewForm, font=("Bahnschrift Semibold", 10), width = 15, text = 'Search', command = Functions.Search)
        resetbutton = Button(LeftViewForm, font=("Bahnschrift Semibold", 10), width = 15, text = 'Reset Search', command = Functions.Reset)
        addbutton = Button(LeftViewForm, font=("Bahnschrift Semibold", 10), width = 15, text = 'Add Item', command =Forms.ShowAddNew)
        editbutton = Button(LeftViewForm, font=("Bahnschrift Semibold", 10), width = 15, text = 'Edit Item', command =Forms.ShowEdit)
        deletebutton = Button(LeftViewForm, font=("Bahnschrift Semibold", 10), width = 15, text = 'Delete Item', command = Functions.Delete)
    ...
    #Treeview 
        ...
        tree = ttk.Treeview(RightViewForm, style="mystyle.Treeview",
                            columns=("Item_ID", "ITEM_NAME", "ITEM_QTY", "ITEMS_IN_USE",
...
##        tree.heading('ACC_NAME', text="Account Name", anchor='center')
        ...
        tree.column('#2', stretch=NO, minwidth=0, width=195)
...
class Functions:
    ...
    def AddNew():
        Data.Database()      
        cursor.execute("INSERT INTO `Inventory` (ITEM_NAME, ITEM_QTY, ITEMS_IN_USE, ITEMS_NOT_IN_USE) VALUES (?, ?, ?, ?)",
                       (str(ITEM_NAME.get()), int(ITEM_QTY.get()), int(ITEMS_IN_USE.get()),  (int(ITEM_QTY.get())-int(ITEMS_IN_USE.get()))))
        conn.commit()
        ITEM_NAME.set("")
        ITEM_QTY.set("")
        ITEMS_IN_USE.set("")
    ...
    def Edit():
        if not tree.selection():
            ...
            result = tkMessageBox.askquestion('Store Inventory System', 'Are you sure you want to edit this record?', icon="warning")
            if result == 'yes':
                curItem = tree.focus()
                ...
                cursor.execute("INSERT INTO `Inventory` (ACCITEM_NAME, ITEM_QTY, ITEMS_IN_USE, ITEMS_NOT_IN_USE) VALUES(?, ?, ?, ?)",
                       (str(ITEM_NAME.get()), int(ITEM_QTY.get()), int(ITEMS_IN_USE.get()),  (int(ITEM_QTY.get())-int(ITEMS_IN_USE.get()))))
                conn.commit()
                ITEM_NAME.set("")
                ITEM_QTY.set("")
    ...
    def Login(event=None):
        ...
        if USERNAME.get == "" or PASSWORD.get() == "":
            ...
            cursor.execute("SELECT * FROM `Accounts` WHERE `ACCOUNT_NAME` = ? AND `Password` = ?", (USERNAME.get(), PASSWORD.get()))
...
b2 = Button(root, font=("Bahnschrift Semibold", 10), text = 'Create Account', height = 2, width = 30,command = Forms.ShowCreateForm)
